 ...
 * Licensed under the MIT (http://www.opensource.org/licenses/mit-license.php) license.
 */
Raphael.fn.g.linechart=function(O,N,b,c,v,u,H){function G(x,ae){var i=x.length/ae,y=0,a=i,Y=0,X=[];while(y<x.length){a--;if(a<0){Y+=x[y]*(1+a);X.push(Y/i);Y=x[y++]*-a;a+=i}else{Y+=x[y++]}}return X}function R(j,i,aj,ah,ae,Y){var y=(aj-j)/2,x=(ae-aj)/2,ak=Math.atan((aj-j)/Math.abs(ah-i)),ai=Math.atan((ae-aj)/Math.abs(ah-Y));ak=i<ah?Math.PI-ak:ak;ai=Y<ah?Math.PI-ai:ai;var X=Math.PI/2-((ak+ai)%(Math.PI*2))/2,am=y*Math.sin(X+ak),ag=y*Math.cos(X+ak),al=x*Math.sin(X+ai),af=x*Math.cos(X+ai);return{x1:aj-am,y1:ah+ag,x2:aj+al,y2:ah+af}}H=H||{};if(!this.raphael.is(v[0],"array")){v=[v]}if(!this.raphael.is(u[0],"array")){u=[u]}var n=H.gutter||10,w=Math.max(v[0].length,u[0].length),p=H.symbol||"",S=H.colors||Raphael.fn.g.colors,M=this,s=null,m=null,ab=this.set(),T=[];for(var aa=0,J=u.length;aa<J;aa++){w=Math.max(w,u[aa].length)}var ac=this.set();for(aa=0,J=u.length;aa<J;aa++){if(H.shade){ac.push(this.path().attr({stroke:"none",fill:S[aa],opacity:H.nostroke?1:0.3}))}if(u[aa].length>b-2*n){u[aa]=G(u[aa],b-2*n);w=b-2*n}if(v[aa]&&v[aa].length>b-2*n){v[aa]=G(v[aa],b-2*n)}}var V=Array.prototype.concat.apply([],v),U=Array.prototype.concat.apply([],u),q=this.g.snapEnds(Math.min.apply(Math,V),Math.max.apply(Math,V),v[0].length-1);if(H.clip){var B=H.minx||q.from,l=H.maxx||q.to,L=this.g.snapEnds(Math.min.apply(Math,U),Math.max.apply(Math,U),u[0].length-1),z=H.miny||L.from,k=H.maxy||L.to}else{var B=H.minx&&Math.min(H.minx,q.from)||q.from,l=H.maxx&&Math.max(H.maxx,xdimt.to)||q.to,L=this.g.snapEnds(Math.min.apply(Math,U),Math.max.apply(Math,U),u[0].length-1),z=H.miny&&Math.min(H.miny,L.from)||L.from,k=H.maxy&&Math.max(H.maxy,L.to)||L.to}kx=(b-n*2)/((l-B)||1),ky=(c-n*2)/((k-z)||1);var K=this.set(),W=this.set(),o;for(aa=0,J=u.length;aa<J;aa++){if(!H.nostroke){K.push(o=this.path().attr({stroke:S[aa],"stroke-width":H.width||2,"stroke-linejoin":"round","stroke-linecap":"round","stroke-dasharray":H.dash||""}))}var e=this.raphael.is(p,"array")?p[aa]:p,E=this.set();T=[];for(var Z=0,t=u[aa].length;Z<t;Z++){var g=O+n+((v[aa]||v[0])[Z]-B)*kx,f=N+c-n-(u[aa][Z]-z)*ky;(Raphael.is(e,"array")?e[Z]:e)&&E.push(this.g[Raphael.fn.g.markers[this.raphael.is(e,"array")?e[Z]:e]](g,f,(H.width||2)*3).attr({fill:S[aa],stroke:"none"}));if(H.smooth){if(Z&&Z!=t-1){var Q=O+n+((v[aa]||v[0])[Z-1]-B)*kx,D=N+c-n-(u[aa][Z-1]-z)*ky,P=O+n+((v[aa]||v[0])[Z+1]-B)*kx,A=N+c-n-(u[aa][Z+1]-z)*ky;var ad=R(Q,D,g,f,P,A);T=T.concat([ad.x1,ad.y1,g,f,ad.x2,ad.y2])}if(!Z){T=["M",g,f,"C",g,f]}}else{T=T.concat([Z?"L":"M",g,f])}}if(H.smooth){T=T.concat([g,f,g,f])}W.push(E);if(H.shade){ac[aa].attr({path:T.concat(["L",g,N+c-n,"L",O+n+((v[aa]||v[0])[0]-B)*kx,N+c-n,"z"]).join(",")})}!H.nostroke&&o.attr({path:T.join(","),"clip-rect":[O+n,N+n,b-2*n,c-2*n].join(",")})}function I(ak){var ah=[];for(var ai=0,am=v.length;ai<am;ai++){ah=ah.concat(v[ai])}ah.sort(function(j,i){return j-i});var an=[],ae=[];for(ai=0,am=ah.length;ai<am;ai++){ah[ai]!=ah[ai-1]&&an.push(ah[ai])&&ae.push(O+n+(ah[ai]-B)*kx)}ah=an;am=ah.length;var Y=ak||M.set();for(ai=0;ai<am;ai++){var y=ae[ai]-(ae[ai]-(ae[ai-1]||O))/2,al=((ae[ai+1]||O+b)-ae[ai])/2+(ae[ai]-(ae[ai-1]||O))/2,a;ak?(a={}):Y.push(a=M.rect(y-1,N,Math.max(al+1,1),c).attr({stroke:"none",fill:"#000",opacity:0}));a.values=[];a.symbols=M.set();a.y=[];a.x=ae[ai];a.axis=ah[ai];for(var ag=0,aj=u.length;ag<aj;ag++){an=v[ag]||v[0];for(var af=0,x=an.length;af<x;af++){if(an[af]==ah[ai]){a.values.push(u[ag][af]);a.y.push(N+c-n-(u[ag][af]-z)*ky);a.symbols.push(ab.symbols[ag][af])}}}ak&&ak.call(a)}!ak&&(s=Y)}function F(ai){var ae=ai||M.set(),a;for(var ag=0,ak=u.length;ag<ak;ag++){for(var af=0,ah=u[ag].length;af<ah;af++){var y=O+n+((v[ag]||v[0])[af]-B)*kx,aj=O+n+((v[ag]||v[0])[af?af-1:1]-B)*kx,x=N+c-n-(u[ag][af]-z)*ky;ai?(a={}):ae.push(a=M.circle(y,x,Math.abs(aj-y)/2).attr({stroke:"none",fill:"#000",opacity:0}));a.x=y;a.y=x;a.value=u[ag][af];a.line=ab.lines[ag];a.shade=ab.shades[ag];a.symbol=ab.symbols[ag][af];a.symbols=ab.symbols[ag];a.axis=(v[ag]||v[0])[af];ai&&ai.call(a)}}!ai&&(m=ae)}var C=this.set();if(H.axis){var h=(H.axis+"").split(/[,\s]+/);+h[0]&&C.push(this.g.axis(O+n,N+n,b-2*n,B,l,H.axisxstep||Math.floor((b-2*n)/20),2,H.axisxlabels||null,H.axisxtype||"t"));+h[1]&&C.push(this.g.axis(O+b-n,N+c-n,c-2*n,z,k,H.axisystep||Math.floor((c-2*n)/20),3,H.axisylabels||null,H.axisytype||"t"));+h[2]&&C.push(this.g.axis(O+n,N+c-n,b-2*n,B,l,H.axisxstep||Math.floor((b-2*n)/20),0,H.axisxlabels||null,H.axisxtype||"t"));+h[3]&&C.push(this.g.axis(O+n,N+c-n,c-2*n,z,k,H.axisystep||Math.floor((c-2*n)/20),1,H.axisylabels||null,H.axisytype||"t"))}ab.push(K,ac,W,C,s,m);ab.lines=K;ab.shades=ac;ab.symbols=W;ab.axis=C;ab.hoverColumn=function(i,a){!s&&I();s.mouseover(i).mouseout(a);return this};ab.clickColumn=function(a){!s&&I();s.click(a);return this};ab.hrefColumn=function(X){var Y=M.raphael.is(arguments[0],"array")?arguments[0]:arguments;if(!(arguments.length-1)&&typeof X=="object"){for(var a in X){for(var j=0,y=s.length;j<y;j++){if(s[j].axis==a){s[j].attr("href",X[a])}}}}!s&&I();for(j=0,y=Y.length;j<y;j++){s[j]&&s[j].attr("href",Y[j])}return this};ab.hover=function(i,a){!m&&F();m.mouseover(i).mouseout(a);return this};ab.click=function(a){!m&&F();m.click(a);return this};ab.each=function(a){F(a);return this};ab.eachColumn=function(a){I(a);return this};return ab};
/*
 * g.Raphael 0.4.1 - Charting library, based on Raphaël
 *
 * Copyright (c) 2009 Dmitry Baranovskiy (http://g.raphaeljs.com)
 * Licensed under the MIT (http://www.opensource.org/licenses/mit-license.php) license.
 */
Raphael.fn.g.piechart=function(j,g,y,c,u){u=u||{};var t=this,v=[],n=this.set(),x=this.set(),q=this.set(),D=[],H=c.length,I=0,M=0,L=0,f=9,J=true;var e=0;for(var E=0;E<H;E++){e+=c[E]}var K=false;var G=-1;for(var E=0;E<H;E++){if(e==c[E]){K=true;G=E;break}}if(H==1||K==true){for(var E=0;E<H;E++){var l=0.1;if(E==G){l=y}q.push(this.circle(j,g,l).attr({fill:u.colors&&u.colors[E]||this.g.colors[E],stroke:u.stroke||"#fff","stroke-width":u.strokewidth==null?1:u.strokewidth}));n.push(this.circle(j,g,l).attr({href:u.href?u.href[E]:null}).attr(this.g.shim));c[E]={value:c[E],order:E,valueOf:function(){return this.value}};q[E].middle={x:j,y:g};q[E].mangle=180}M=c[G]}else{function C(P,O,h,R,N,W){var T=Math.PI/180,p=P+h*Math.cos(-R*T),i=P+h*Math.cos(-N*T),Q=P+h/2*Math.cos(-(R+(N-R)/2)*T),V=O+h*Math.sin(-R*T),U=O+h*Math.sin(-N*T),w=O+h/2*Math.sin(-(R+(N-R)/2)*T),S=["M",P,O,"L",p,V,"A",h,h,0,+(Math.abs(N-R)>180),1,i,U,"z"];S.middle={x:Q,y:w};return S}for(var E=0;E<H;E++){M+=c[E];c[E]={value:c[E],order:E,valueOf:function(){return this.value}}}c.sort(function(i,h){return h.value-i.value});for(E=0;E<H;E++){if(J&&c[E]*360/M<=1.5){f=E;J=false}if(E>f){J=false;c[f].value+=c[E];c[f].others=true;L=c[f].value}}H=Math.min(f+1,c.length);L&&c.splice(H)&&(c[f].others=true);for(E=0;E<H;E++){var B=c[E].order;var k=I-360*c[E]/M/2;if(!E){I=90-k;k=I-360*c[E]/M/2}if(u.init){var o=C(j,g,1,I,I-360*c[E]/M).join(",")}var A=C(j,g,y,I,I-=360*c[E]/M);var z=this.path(u.init?o:A).attr({fill:u.colors&&u.colors[B]||this.g.colors[B]||"#666",stroke:u.stroke||"#fff","stroke-width":(u.strokewidth==null?1:u.strokewidth),"stroke-linejoin":"round"});z.value=c[E];z.middle=A.middle;z.mangle=k;v.push(z);q.push(z);u.init&&z.animate({path:A.join(",")},(+u.init-1)||1000,">")}for(E=0;E<H;E++){z=t.path(v[E].attr("path")).attr(this.g.shim);var B=c[E].order;u.href&&u.href[B]&&z.attr({href:u.href[B]});n.push(z)}}x.hover=function(N,p){p=p||function(){};var w=this;for(var h=0;h<H;h++){(function(O,P,i){var Q={sector:O,cover:P,cx:j,cy:g,mx:O.middle.x,my:O.middle.y,mangle:O.mangle,r:y,value:c[i],total:M,label:w.labels&&w.labels[i]};P.mouseover(function(){N.call(Q)}).mouseout(function(){p.call(Q)})})(q[h],n[h],h)}return this};x.each=function(w){var p=this;for(var h=0;h<H;h++){(function(N,O,i){var P={sector:N,cover:O,cx:j,cy:g,x:N.middle.x,y:N.middle.y,mangle:N.mangle,r:y,value:c[i],total:M,label:p.labels&&p.labels[i]};w.call(P)})(q[h],n[h],h)}return this};x.click=function(w){var p=this;for(var h=0;h<H;h++){(function(N,O,i){var P={sector:N,cover:O,cx:j,cy:g,mx:N.middle.x,my:N.middle.y,mangle:N.mangle,r:y,value:c[i],total:M,label:p.labels&&p.labels[i]};O.click(function(){w.call(P)})})(q[h],n[h],h)}return this};x.inject=function(h){h.insertBefore(n[0])};var b=function(S,N,w,p){var W=j+y+y/5,V=g,R=V+10;S=S||[];p=(p&&p.toLowerCase&&p.toLowerCase())||"east";w=t.g.markers[w&&w.toLowerCase()]||"disc";x.labels=t.set();for(var Q=0;Q<H;Q++){var X=q[Q].attr("fill"),O=c[Q].order,P;c[Q].others&&(S[O]=N||"Others");S[O]=t.g.labelise(S[O],c[Q],M);x.labels.push(t.set());x.labels[Q].push(t.g[w](W+5,R,5).attr({fill:X,stroke:"none"}));x.labels[Q].push(P=t.text(W+20,R,S[O]||c[O]).attr(t.g.txtattr).attr({fill:u.legendcolor||"#000","text-anchor":"start"}));n[Q].label=x.labels[Q];R+=P.getBBox().height*1.2}var T=x.labels.getBBox(),U={east:[0,-T.height/2],west:[-T.width-2*y-20,-T.height/2],north:[-y-T.width/2,-y-T.height-10],south:[-y-T.width/2,y+10]}[p];x.labels.translate.apply(x.labels,U);x.push(x.labels)};if(u.legend){b(u.legend,u.legendothers,u.legendmark,u.legendpos)}x.push(q,n);x.series=q;x.covers=n;var s=t.width,F=t.height,m=x.getBBox(),a=[(s-m.width)/2-m.x,(F-m.height)/2-m.y];j+=a[0];g+=a[1];x.translate.apply(x,a);return x};